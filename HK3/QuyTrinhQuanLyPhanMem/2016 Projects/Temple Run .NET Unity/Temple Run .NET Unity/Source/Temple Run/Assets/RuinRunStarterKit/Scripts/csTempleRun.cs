	...
	: MonoBehaviour
{
	...
	//Determines type of drag action
	...
	//The tolal coin of all game
	private float 
			m_totalCoinCollected;
	...
	//How long the item effects to player
	private float m_shieldTimer = 0.0f;
	private float m_coinMagetTimer = 0.0f;
	
	//Maximum value of the item
	private float m_maxShieldTimer = 0.0f;
	private float m_maxCoinMagetTimer = 0.0f;
	private float m_maxCoinBonus = 0.0f;
	private float m_maxPointBonus = 0.0f;

	//Current item that player is using
	private GameObject m_currentItem = null;
	
	// Set when the player dies, so the GUI can explain what happened (or insult the player...)
	private Texture m_dieReason;
	
	//Detect type of drag action
	private dragType m_dragAction;
		
	//Detect state of game
	private stateGame m_StateGame;
	...
	void Update ()
	{	
		if (m_StateGame == stateGame.Playing && m_player != null) {
			UpdatePlayer ();
	...
	public void OnGUI ()
	{
		...
		if (m_StateGame == stateGame.Start) {
			...
			if (GUI.Button (screenRect (0.4f, 0.8f, 0.2f, 0.1f), m_startGame)) {
...
//		
		...
		if (m_StateGame == stateGame.Playing && m_player == null) {
			...
			//m_StateGame = stateGame.Dead;
			if (m_resetTimer > 5.0f) {
				m_resetTimer = 5.0f;
				centeredStyle.normal.textColor = Color.red;

				
				GUI.DrawTexture (screenRect (0f, 0f, 1f, 1f), m_gameOver);
					
				GUI.DrawTexture (screenRect (0.2f, 0.2f, 0.6f, 0.2f), m_LOGO);
	
				GUI.DrawTexture (screenRect (0.073f, 0.15f, 0.85f, 0.28f), m_dieReason);
					
				centeredStyle.fontStyle = FontStyle.Bold;
				centeredStyle.fontSize = (int)(Screen.height * 0.07);
				GUI.Label (screenRect (0.35f, 0.49f, 0.3f, 0.5f), m_Point. ToString ());
				
				centeredStyle.fontSize = (int)(Screen.height * 0.05);
				centeredStyle.fontStyle = FontStyle.BoldAndItalic;
				centeredStyle.normal.textColor = new Color ((float)165 / 255, (float)42 / 255, (float)42 / 255);
				GUI.Label (screenRect (0.4f, 0.56f, 0.3f, 0.5f), m_distanceRun. ToString () + "M");
					
				GUI.Label (screenRect (0.4f, 0.61f, 0.3f, 0.5f), m_coinsCollected.ToString ());
				PlayerPrefs.SetFloat ("totalcoin", m_totalCoinCollected);
				GUI.backgroundColor = new Color (0, 0, 0, 0);
				if (GUI.Button (screenRect (0.08f, 0.75f, 0.83f, 0.13f), m_runAgain, buttonStyle)) {	
					DestroyAllCells ();
					CreateCells ();			
					CreatePlayer ();
					m_resetTimer = 0.0f;
				...
				if (GUI.Button (new Rect(0, Screen.height - 30, 70, 30), "Upgrade", buttonStyle))
				{
		...
		if (m_StateGame == stateGame.Playing && m_player != null) {
			...
			centeredStyle.fontSize = (int)(Screen.height * 0.04);
			centeredStyle.normal.textColor = Color.yellow;
			centeredStyle.alignment = TextAnchor.UpperLeft;
			GUI.Label (screenRect (0.0f, 0.01f, 0.7f, 0.05f), "Distance:" + m_distanceRun + "M");
			GUI.Label (screenRect (0.0f, 0.07f, 0.7f, 0.05f), "Coins:" + m_coinsCollected.ToString ());
			GUI.Label (screenRect (0.0f, 0.15f, 0.7f, 0.05f), "Points:" + m_Point. ToString ());
			m_Point = m_distanceRun * 5 + m_coinsCollected * 10;			
		}
	...
	private void UpdateCoins ()
	{
		...
		for (int c = 0; c<m_cells.Count; c++) {	
			for (int k=m_cells[c].m_coins.Count-1; k>=0; k--) {
				float _distance = Vector3.Distance (m_cells [c].m_coins [k].transform.position, m_player.transform.position);
				if (m_coinMagetTimer > 0.0f) {
					if (_distance < 2.0f && _distance >= 1.0f) {
						Vector3 _temp = (m_player.transform.position - m_cells [c].m_coins [k].transform.position) / 20;
						m_cells [c].m_coins [k].transform.position += _temp;
	...
	private void UpdateMines ()
	{
		...
		if (m_coinMagetTimer == 0.0f && m_shieldTimer == 0.0f) {
			Destroy (m_currentItem);
			m_currentItem = null;
		}
		
		if (m_currentItem != null) {
			m_currentItem.transform.position = m_player.transform.position + Vector3.up * 0.8f;
			
		}
		
		//Count timer of item effection
		m_shieldTimer = Mathf.Max (0.0f, m_shieldTimer - Time.deltaTime);
		m_coinMagetTimer = Mathf.Max (0.0f, m_coinMagetTimer - Time.deltaTime);
		
		for (int c = 0; c<m_cells.Count; c++) {
			for (int k=m_cells[c].m_mines.Count-1; k>=0; k--) {
				if (Vector3.Distance (m_cells [c].m_mines [k].transform.position, m_player.transform.position) < 1.0f) {	
					AudioSource.PlayClipAtPoint (m_getItem, m_player.transform.position);						
					GameObject _temp = m_cells [c].m_mines [k];
					_temp.transform.localScale = Vector3.one * 0.1f;
					UpdateItem _itemClass = (UpdateItem)_temp.GetComponent (typeof(UpdateItem));
					switch (_itemClass.CurrentType) {
					case 0:
						m_coinsCollected += m_maxCoinBonus;
						m_totalCoinCollected += m_maxCoinBonus;
						break;
					case 1:
						m_shieldTimer = m_maxShieldTimer;
						m_currentItem = _temp;
						m_coinMagetTimer = 0.0f;
						break;
					case 2:
	...
	private void CreatePlayer ()
	{
		...
		m_maxCoinBonus = Mathf.Max (5.0f, PlayerPrefs.GetFloat ("coinbonus"));
		m_maxCoinMagetTimer = Mathf.Max (2.0f, PlayerPrefs.GetFloat ("coinmaget"));
		m_maxPointBonus = Mathf.Max (5.0f, PlayerPrefs.GetFloat ("pointbonus"));
		m_maxShieldTimer = Mathf.Max (2.0f, PlayerPrefs.GetFloat ("shield"));
		...
		m_coinsCollected = 0;
		m_totalCoinCollected = PlayerPrefs.GetFloat ("totalcoin");
		m_totalCoinCollected = PlayerPrefs.GetFloat("totalcoin");
		if (m_playerRagDoll != null)
			Destroy (m_playerRagDoll);
	}
...
//		}		
		...
		// ****************************************************************************************
		// INPUT
		//Detect the type of the drag action
		
		m_dragAction = dragType.None;
		if (Input.touchCount > 0) {
			Touch mytouch = Input.GetTouch (0);
		
			if (mytouch.phase == TouchPhase.Moved) {
				if (Mathf.Abs (mytouch.deltaPosition.y / mytouch.deltaPosition.x) <= 1) {
					if (mytouch.deltaPosition.x > 0) {
						m_dragAction = dragType.Right;
					} else {
						m_dragAction = dragType.Left;
					}
				} else {
					if (mytouch.deltaPosition.y > 0) {
						m_dragAction = dragType.Up;
					} else {
						m_dragAction = dragType.Down;
					}
				}
			}
		}
		...
		if ((Input.GetKeyDown (KeyCode.LeftArrow) || m_dragAction == dragType.Left) && m_playerJump <= -1.0f && m_playerSlide <= 0.0f) {
			if (m_playerDirection == enCellDir.North)
				m_playerNextDirection = enCellDir.West;
			if (m_playerDirection == enCellDir.East)
				m_playerNextDirection = enCellDir.North;
			if (m_playerDirection == enCellDir.South)
				m_playerNextDirection = enCellDir.East;
			if (m_playerDirection == enCellDir.West)
				m_playerNextDirection = enCellDir.South;
		}

		if ((Input.GetKeyDown (KeyCode.RightArrow) || m_dragAction == dragType.Right) && m_playerJump <= -1.0f && m_playerSlide <= 0.0f) {
			if (m_playerDirection == enCellDir.North)
				m_playerNextDirection = enCellDir.East;
			if (m_playerDirection == enCellDir.East)
				m_playerNextDirection = enCellDir.South;
		...
		if ((Input.GetKeyDown (KeyCode.DownArrow) || m_dragAction == dragType.Down) && m_playerJump <= -1.0f && m_playerSlide <= 0.0f) {
			m_playerSlide = 1.0f;
			m_player.animation.Play ("slide_fake");
		}			 
		
		if ((Input.GetKeyDown (KeyCode.UpArrow) || m_dragAction == dragType.Up) && m_playerJump <= -1.0f && m_playerSlide <= 0.0f) {
		...
		if (m_playerCell.CellType == enCellType.JumpGap && m_playerJump <= -1.0f) {
			if (m_playerTimer > 0.3f && m_playerTimer < 0.7f) {
				if (m_playerTimer < 0.4f)
					DoRagDoll (false, false, m_fallAudio);
				if (m_playerTimer >= 0.4f)
					DoRagDoll (false, false, m_splatAudio);

				m_dieReason = m_fallWater;
			}
		}
		...
		if (m_playerDirection == enCellDir.North)
			m_player.transform.rotation = Quaternion.Euler (xRot, 000, 0);
		if (m_playerDirection == enCellDir.South)
			m_player.transform.rotation = Quaternion.Euler (xRot, 180, 0);
		if (m_playerDirection == enCellDir.East)
			m_player.transform.rotation = Quaternion.Euler (xRot, 090, 0);
		if (m_playerDirection == enCellDir.West)
			m_player.transform.rotation = Quaternion.Euler (xRot, 270, 0);
		...
		m_tilt += Input.acceleration.x * 0.07f;
	...
	private void CreateScenery (stCell cell)
	{
		...
		if (m_cells.IndexOf (cell) == m_cells.Count - 1)
			return;
		...
		if (Random.Range (0.0f, 1.0f) > 0.25f) {
			...
			if (cell.CellDirection == enCellDir.North || cell.CellDirection == enCellDir.South)
				archRot = Quaternion.Euler (0, 0, 0);
			if (cell.CellDirection == enCellDir.East || cell.CellDirection == enCellDir.West)
				archRot = Quaternion.Euler (0, 90, 0);
			...
			if (cell.CellTheme == enCellTheme.Bridge)
				pos.y = m_tileBaseHeightRuin;
		...
		if (cell.NeighbourW == null && Random.Range (0.0f, 1.0f) > 0.25f) {
			...
			switch (sceneryID) {
			case 0:
				sceneryPrefab = m_Rock1;
				break;
			case 1:
				sceneryPrefab = m_Rock2;
				break;
			case 2:
				sceneryPrefab = m_Rock3;
				break;
			case 3:
				sceneryPrefab = m_Rock4;
				break;
			case 4:
				sceneryPrefab = m_Column;
				break;					
			case 5:
				sceneryPrefab = m_Tree;
				break;					
			case 6:
				sceneryPrefab = m_Ruin1;
				break;							
			case 7:
				sceneryPrefab = m_Ruin2;
				break;					
			}
		...
		if (Random.Range (0.0f, 1.0f) > 0.5f && m_cells.IndexOf (cell) > 2) {	
			...
			if (cell.CellType == enCellType.Run) {
				...
				if (!isCornerCell (cell)) {
					// Create the landmine.
					GameObject landMine = (GameObject)Instantiate (m_LandMine, cell.CellPosition + Vector3.up * (cell.CellTheme == enCellTheme.Stone ? 0.8f : 0.1f) + Vector3.up, Quaternion.identity);					
	...
	private void CreateCoins (stCell cell)
	{
		...
		if (Random.Range (0.0f, 1.0f) > 0.25f) {
			...
			if (cell.CellType == enCellType.LedgeLeft && cell.CellDirection == enCellDir.North)
				r = -1;
			if (cell.CellType == enCellType.LedgeRight && cell.CellDirection == enCellDir.North)
				r = 1;
			
			if (cell.CellType == enCellType.LedgeLeft && cell.CellDirection == enCellDir.East)
				r = 1;
			if (cell.CellType == enCellType.LedgeRight && cell.CellDirection == enCellDir.East)
				r = -1;
			...
			for (float k=0.5f; k<1.0f; k+=0.1f) {
				...
				if (cell.CellType == enCellType.JumpGap || cell.CellType == enCellType.JumpObstacle) {
					...
					if (cell.CellDirection == enCellDir.North)
						pos.z -= (m_cellSpacing / 5.0f);
					if (cell.CellDirection == enCellDir.East)
						pos.x -= (m_cellSpacing / 5.0f);
				}
	...
	private void CreateCellModel (stCell cell)
	{
		...
		case enCellType.JumpObstacle:
			...
			if (cell.CellTheme == enCellTheme.Stone)
				prefabToInstantiate = m_ITile_ObstacleJump;
			if (cell.CellTheme == enCellTheme.Bridge)
				prefabToInstantiate = m_Bridge_ITile_ObstacleJump;
		...
		case enCellType.DuckObstacle:
			...
			if (cell.CellTheme == enCellTheme.Stone)
				prefabToInstantiate = m_ITile_ObstacleDuck;
			if (cell.CellTheme == enCellTheme.Bridge)
				prefabToInstantiate = m_Bridge_ITile_ObstacleDuck;
