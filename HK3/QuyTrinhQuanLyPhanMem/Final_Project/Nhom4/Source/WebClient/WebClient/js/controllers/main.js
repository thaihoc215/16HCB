...
navbarCtrl.$inject = ['$scope', '$state', '$http', 'store', 'httpModule', 'tokenModule'];

function navbarCtrl($scope, $state, $http, store, httpModule, tokenModule) {
    ...
    var c = $scope.user.name.split('')[0].toLowerCase();;
    $scope.user.avatar = 'https://ssl.gstatic.com/bt/C3341AA7A1A076756462EE2E5CD71C11/avatars/avatar_tile_' + c + '_56.png';
    //Switch mode 2 authentication
    $scope.switchAuthentication = function() {

        if ($scope.icon === 'fa fa-shield') {

            var req = {
                method: 'GET',
                url: httpModule + '/api/web/Authen/' + user.username + '/false/' + tokenModule,
                headers: {
            ...
            $http(req).then(function(res){
                ...
                if (status === 200) {
                    $scope.icon = 'fa fa-unlock';
                    user.icon = $scope.icon;
                    store.set('user', user);
                } else {
            ...
            var req = {
                method: 'GET',
                url: httpModule + '/api/web/Authen/' + user.username + '/true/' + tokenModule,
                headers: {
                    'Content-Type': 'application/json'
                }
            };
            ...
            $http(req).then(function(res){
                ...
                if (status === 200) {
                    ...
                    $scope.icon = 'fa fa-shield';
                    
                    user.icon = $scope.icon;
                    user.secretKey = data.secretKey;
                    store.set('user', user);
                    $state.go('appSimple.authentication');
                } else {
                    console.log('Access deny!!!');
                }
